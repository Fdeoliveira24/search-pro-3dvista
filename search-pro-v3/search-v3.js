...
if (!window.Logger) {
  ...
  console.info(
    "[Search] Using fallback Logger shim until debug-core-v3.js loads",
  );
}
...
if (window.Logger && typeof window.Logger === 'object') {
  ...
  if (typeof window.Logger.debug !== 'function') {
    window.Logger.debug = function(msg, ...args) { console.debug("[Search] DEBUG:", msg, ...args); };
  }
  if (typeof window.Logger.info !== 'function') {
    window.Logger.info = function(msg, ...args) { console.info("[Search] INFO:", msg, ...args); };
  }
  if (typeof window.Logger.warn !== 'function') {
    window.Logger.warn = function(msg, ...args) { console.warn("[Search] WARN:", msg, ...args); };
  }
  if (typeof window.Logger.error !== 'function') {
    window.Logger.error = function(msg, ...args) { console.error("[Search] ERROR:", msg, ...args); };
  }
}
    ...
    return new Promise((resolve) => {
      ...
      if (
        document.querySelector(
          'link[href="search-pro-v3/css/search-v3.css"]',
        )
      ) {
        resolve();
        return;
...
window.tourSearchFunctions = (function () {
        ...
        switch (e.key) {
          ...
          case "Tab":
            ...
            if (selectedIndex >= 0 && selectedIndex < resultItems.length) {
              ...
              const resultText = selectedResult.querySelector('.result-title')?.textContent || 
  ...
  function _shouldAutoHide() {
    ...
    return (isMobile && _config?.autoHide?.mobile) ||
  ...
  function _resolveAssetUrl(input) {
    ...
    const base = (function () {
      const script = Array.from(document.scripts || []).find(s => /search-v3\.js(\?|$)/.test(s.src));
      if (script?.src) {
    ...
    try {
      ...
      if (overlay.id || label) {
        ...
        if (isProjectedImageElement) {
          console.log(
            `[ELEMENT TYPE DEBUG] Returning ProjectedImage based on name: ${overlayId}`,
          );
          return "ProjectedImage";
        }
      }
...
.result-item .highlight {
  ...
  function _loadBusinessData() {
    ...
    let dataPath;
    const raw = (_config.businessData.businessDataUrl || "").trim();
    if (raw) {
      dataPath = /^https?:\/\//i.test(raw) ? raw : __fromScript(raw);
    } else {
      ...
      dataPath = __fromScript(`${dataDir}/${dataFile}`);
    }

  Logger.info(`Loading business data from: ${dataPath}`);
  console.log("[search-v3] data URLs OK:", { business: dataPath, csv: undefined });
      ...
      try {
        ...
        (function guardOrigin() {
          try {
            ...
            const allowed = Array.isArray(_config?.controlPanel?.allowedOrigins) ? _config.controlPanel.allowedOrigins : [];
    ...
    function _processOverlaysWithSource(
      ...
      overlays.forEach((overlay, overlayIndex) => {
        try {
          ...
          let displayLabel = _getDisplayLabel(
            overlayLabel,
            overlaySubtitle,
            elementTags,
            {
