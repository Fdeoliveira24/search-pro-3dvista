...
class DataSourcesTabHandler {
          ...
          if (businessDataToggle.checked) {
            ...
            if (googleSheetsToggle.checked) {
              ...
              if (mainWarningBanner) {
                ...
                if (messageEl) {
                  messageEl.textContent = "Business Data and Google Sheets cannot be enabled simultaneously. Google Sheets has been disabled.";
                }
          ...
          if (googleSheetsToggle.checked) {
            ...
            if (businessDataToggle.checked) {
              ...
              if (mainWarningBanner) {
                ...
                const messageEl = mainWarningBanner.querySelector("#dataSourcesWarningMessage");
                if (messageEl) {
                  messageEl.textContent = "Google Sheets and Business Data cannot be enabled simultaneously. Business Data has been disabled.";
                }
                // Auto-hide the warning after 5 seconds
                setTimeout(() => {
                  if (mainWarningBanner.style.display !== "none") {
  ...
  updateMutualExclusivityStates(container) {
    try {
      const businessDataToggle = container.querySelector("#useBusinessData");
      const googleSheetsToggle = container.querySelector("#useGoogleSheetData");
      const businessDataSection = container.querySelector('[aria-labelledby="business-data-heading"]');
      const googleSheetsSection = container.querySelector('[aria-labelledby="google-sheets-heading"]');
      ...
      if (businessDataToggle?.checked) {
        ...
        if (mainWarningBanner) {
          ...
          const messageEl = mainWarningBanner.querySelector("#dataSourcesWarningMessage");
          if (messageEl) {
            messageEl.textContent = "No data source is currently enabled. Only one external data source can be enabled at a time. Enable either Business Data or Google Sheets/CSV to use external data.";
          }
        }
      }
    } catch (error) {
...
resetBusinessDataToDefaults(container) {
  try {
    ...
    this.core.config.businessData.useBusinessData = defaults.useBusinessData !== undefined ? defaults.useBusinessData : false;
    this.core.config.businessData.replaceTourData = defaults.replaceTourData !== undefined ? defaults.replaceTourData : false;
    this.core.config.businessData.includeStandaloneEntries = defaults.includeStandaloneEntries !== undefined ? defaults.includeStandaloneEntries : false;
    ...
    if (this.core.showToast) {
      this.core.showToast("success", "Reset Complete", "Business Data settings reset to defaults");
    }
  ...
  resetGoogleSheetsToDefaults(container) {
    try {
      ...
      this.core.config.googleSheets.useGoogleSheetData = defaults.useGoogleSheetData !== undefined ? defaults.useGoogleSheetData : false;
      this.core.config.googleSheets.includeStandaloneEntries = defaults.includeStandaloneEntries !== undefined ? defaults.includeStandaloneEntries : false;
      this.core.config.googleSheets.useAsDataSource = defaults.useAsDataSource !== undefined ? defaults.useAsDataSource : false;
      this.core.config.googleSheets.googleSheetUrl = defaults.googleSheetUrl || "";
      this.core.config.googleSheets.useLocalCSV = defaults.useLocalCSV !== undefined ? defaults.useLocalCSV : false;
      ...
      if (this.core.showToast) {
        this.core.showToast("success", "Reset Complete", "Google Sheets settings reset to defaults");
      }
  ...
  resetCSVParsingToDefaults(container) {
    try {
      ...
      this.core.config.googleSheets.csvOptions.header = defaults.header !== undefined ? defaults.header : true;
      this.core.config.googleSheets.csvOptions.skipEmptyLines = defaults.skipEmptyLines !== undefined ? defaults.skipEmptyLines : true;
      this.core.config.googleSheets.csvOptions.dynamicTyping = defaults.dynamicTyping !== undefined ? defaults.dynamicTyping : true;
  ...
  resetCachingToDefaults(container) {
    try {
      ...
      this.core.config.googleSheets.caching.enabled = defaults.enabled !== undefined ? defaults.enabled : false;
      this.core.config.googleSheets.caching.timeoutMinutes = defaults.timeoutMinutes || 60;
      this.core.config.googleSheets.caching.storageKey = defaults.storageKey || "tourGoogleSheetsData";
  ...
  onFormChange(input) {
    try {
      ...
      } else if (fieldName === "googleSheets.useGoogleSheetData" || fieldName === "useGoogleSheetData") {
        this.updateMutualExclusivityStates(document.getElementById("data-sources-panel"));
