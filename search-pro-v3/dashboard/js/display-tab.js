...
setupResetButtonHandlers(container) {
  ...
  toggleIconConfigurationSections(isEnabled) {
    try {
      ...
      console.log(
        `🎨 Icon configuration sections ${isEnabled ? "shown" : "hidden"}`,
      );
    } catch (error) {
  ...
  setupDisplayBehaviorHandlers(container = document) {
    try {
      ...
      const toggleSwitches =
        displayBehaviorSection.querySelectorAll(".toggle-input");
      ...
      console.error(
        "🚨 Security: Error setting up display behavior handlers:",
        error,
      );
    }
  }
  ...
  setupThumbnailHandlers(container = document) {
    try {
      ...
      const enableThumbnailsToggle =
        thumbnailSection.querySelector("#enableThumbnails");
      ...
      const colorInput = thumbnailSection.querySelector(
      ...
      const hexInput = thumbnailSection.querySelector(
        "#thumbnailBorderColor + .color-hex-input",
      );
      if (hexInput) {
        hexInput.addEventListener("input", (e) => {
  ...
  handleIconSystemToggle() {
    try {
      ...
      const fontAwesomeCdnSection = document.querySelector(
        "#fontAwesomeCdnSection",
      );
  ...
  getConsistentEmojiSet(elementType) {
    try {
      ...
      if (window.EmojiLibrary) {
        ...
        if (
          elementType &&
          typeof window.EmojiLibrary.getElementEmojis === "function"
        ) {
          const elementEmojis =
            window.EmojiLibrary.getElementEmojis(elementType);
  ...
  highlightSelectedIcon(element, gridContainer, selectedIcon = null) {
    try {
      ...
      if (selectedIcon) {
        const selectedItem = gridContainer.querySelector(
        ...
        if (selectedItem) {
          ...
          if (selectedItem.offsetTop > gridContainer.clientHeight) {
            gridContainer.scrollTop =
              selectedItem.offsetTop - gridContainer.clientHeight / 2;
          }
        }
      }
    } catch (error) {
  ...
  populateIconForm(container) {
    try {
      ...
      const iconColorHex = container.querySelector(
        "#iconColor + .color-hex-input",
      );
      ...
      const iconOpacityValue = container.querySelector(
        "#iconOpacity + .range-value",
      );
  ...
  setupGlobalDefaultPathHandler(container) {
    try {
      ...
      console.error(
        "🚨 Security: Error setting up global default path handler:",
        error,
      );
    }
  }
  ...
  updateRangeValue(rangeInput) {
    try {
      ...
      console.log(
        `🎚️ Range value updated: ${rangeInput.id} = ${rangeInput.value}`,
      );
    } catch (error) {
  ...
  populateThumbnailForm(container) {
    try {
      ...
      const borderRadiusValue = container.querySelector(
        "#thumbnailBorderRadius + .range-value",
      );
      ...
      const borderWidthValue = container.querySelector(
        "#thumbnailBorderWidth + .range-value",
      );
      ...
      const borderColorHex = container.querySelector(
        "#thumbnailBorderColor + .color-hex-input",
      );
  ...
  loadExistingImagePreview(imagePath, container) {
    try {
      ...
      console.error(
        "🚨 Security: Error loading existing image preview:",
        error,
      );
    }
  }
  ...
  resetLabelFallbackToDefaults() {
    try {
      ...
      console.error(
        "🚨 Security: Error resetting label fallback options:",
        error,
      );
    }
  }
    ...
    try {
      ...
      if (this.core.showToast) {
        this.core.showToast(
          "success",
          "Reset Complete",
          "Label customization reset to defaults",
        );
      }
  ...
  handleColorInputChange(input) {
    try {
      const hexInput = input
        .closest(".color-input-wrapper")
        .querySelector(".color-hex-input");
      if (hexInput) {
  ...
  handleDisplayBehaviorToggle(toggle) {
    try {
      ...
      console.error(
        "🚨 Security: Error handling display behavior toggle:",
        error,
      );
    }
  }
  ...
  onFormChange(input, skipInterdependencyCheck = false) {
    try {
      ...
      console.error(
        "🚨 Security: Error handling Display tab form change:",
        error,
      );
    }
  }
  ...
  resetToDefaults() {
    try {
      ...
      this.core.safeSetNestedProperty(
        this.core.config,
        "elementContent.showDescriptions",
        true,
      );
    ...
    try {
      ...
      if (this.core.showToast) {
        this.core.showToast(
          "success",
          "Reset Complete",
          "Display behavior reset to defaults",
        );
      }
    ...
    try {
      ...
      Object.keys(visibilityDefaults).forEach((key) => {
        ...
        const toggle = document.querySelector(
          `[name="displayElements.showFor.${key}"]`,
        );
  ...
  validateField(field) {
    try {
      ...
      console.error(
        "🚨 Security: Error in Display tab field validation:",
        error,
      );
      return false;
    }
  ...
  validateForm(container = document) {
    try {
      ...
      console.log(
        `👁️ Display tab validation: ${isValid ? "PASSED" : "FAILED"}`,
      );
      return isValid;
    } catch (error) {
      ...
      return {
        ...
        sections: {
          display: {
            ...
            showTagsInResults:
              this.core.config.display?.showTagsInResults || true,
            showSubtitlesInResults:
          ...
          thumbnailSettings: {
            ...
            defaultImagePath: 
              this.core.config.thumbnailSettings?.defaultImagePath || "assets/default-thumbnail.jpg",
