...
class ContentTabHandler {
  ...
  populateContentForm(container) {
    try {
      ...
      if (
        containerNamesInput &&
        this.core.config.includeContent?.containerSearch?.containerNames
      ) {
  ...
  setupContainerSearchHandlers(container) {
    try {
      const enableContainerSearchToggle = container.querySelector(
      ...
      console.error(
        "🚨 Security: Error setting up container search handlers:",
        error,
      );
    }
  }
  ...
  handleContainerNamesChange(input) {
    try {
      ...
      console.error(
        "🚨 Security: Error handling container names change:",
        error,
      );
    }
  }
  ...
  setupResetButtonHandlers(container) {
    try {
      ...
      console.error(
        "🚨 Security: Error setting up reset button handlers:",
        error
      );
    }
  }
  ...
  resetContentInclusionToDefaults() {
    try {
      ...
      this.core.config.includeContent.unlabeledWithSubtitles =
        defaults.unlabeledWithSubtitles !== undefined
          ? defaults.unlabeledWithSubtitles
          : true;

      this.core.config.includeContent.unlabeledWithTags =
        defaults.unlabeledWithTags !== undefined
          ? defaults.unlabeledWithTags
          : true;

      this.core.config.includeContent.completelyBlank =
        defaults.completelyBlank !== undefined
          ? defaults.completelyBlank
          : true;
      ...
      console.error(
        "🚨 Security: Error resetting content inclusion options:",
        error,
      );
    }
  }
  ...
  resetElementTypesToDefaults() {
    try {
      const defaults =
        this.core.getDefaultConfig().includeContent?.elements || {};
      ...
      if (this.core.showToast) {
        this.core.showToast(
          "success",
          "Reset Complete",
          "Element type options reset to defaults",
        );
      }
      console.log("🔄 Element type options reset to defaults");
    } catch (error) {
      console.error(
        "🚨 Security: Error resetting element type options:",
        error,
      );
    }
  }
  ...
  resetContainerSearchToDefaults() {
    try {
      const defaults =
        this.core.getDefaultConfig().includeContent?.containerSearch || {};
      ...
      this.core.config.includeContent.containerSearch.enableContainerSearch =
        defaults.enableContainerSearch !== undefined
          ? defaults.enableContainerSearch
          : true;

      this.core.config.includeContent.containerSearch.containerNames =
        Array.isArray(defaults.containerNames)
      ...
      console.error(
        "🚨 Security: Error resetting container search options:",
        error,
      );
    }
  }
  ...
  validateField(field) {
    try {
      ...
      console.error(
        "🚨 Security: Error in Content tab field validation:",
        error,
      );
      return false;
    }
  ...
  validateForm(container = document) {
    try {
      ...
      console.log(
        `📄 Content tab validation: ${isValid ? "PASSED" : "FAILED"}`,
      );
      return isValid;
    } catch (error) {
  ...
  onFormChange(input) {
    try {
      ...
      console.error(
        "🚨 Security: Error handling Content tab form change:",
        error,
      );
    }
  }
  ...
  resetToDefaults() {
    try {
      ...
      if (defaults.includeContent) {
        this.core.config.includeContent = JSON.parse(
          JSON.stringify(defaults.includeContent),
        );
      } else {
  ...
  getConfigSummary() {
    try {
      ...
      console.error(
        "🚨 Security: Error getting Content tab config summary:",
        error,
      );
      return null;
    }
