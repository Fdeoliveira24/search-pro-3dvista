      ...
      fetch(`tabs/${fileId}-tab.html`)
        .then((response) => {
          if (!response.ok) {
            ...
            if (response.status === 403) {
              throw new Error(`Server security policy blocks access to tab files (HTTP 403). This is common with production servers that restrict subdirectory access.`);
            }
            throw new Error(
        ...
        .catch((error) => {
          ...
          try {
            ...
            if (fallbackContent) {
              console.log(`📄 Successfully using fallback content for ${tabId} tab due to server restrictions`);
