...
class FilteringTabHandler {
  ...
  filterMediaIndexInput(input) {
    try {
      ...
      if (originalValue !== filteredValue) {
        ...
        this.showValidationMessage(input, "Invalid characters removed. Only numbers, commas, and spaces are allowed.");
  ...
  validateMediaIndexInput(input) {
    try {
      ...
      if (!isValid) {
        if (!validFormat) {
          this.showValidationMessage(input, "Only numbers, commas, and spaces are allowed (e.g., 0, 1, 5)");
        } else {
  ...
  setupElementTypeControls(container = document) {
    try {
      ...
      const allowedCheckboxes = container.querySelectorAll('#allowedElementTypesCheckboxes input[type="checkbox"]');
      ...
      const blacklistedCheckboxes = container.querySelectorAll('#blacklistedElementTypesCheckboxes input[type="checkbox"]');
  ...
  updateFilterVisibility(container = document) {
    try {
      ...
      if (allowedValuesGroup) {
        allowedValuesGroup.style.display = 
          filterMode === "whitelist" ? "block" : "none";
      }

      if (blacklistedValuesGroup) {
        blacklistedValuesGroup.style.display = 
          filterMode === "blacklist" ? "block" : "none";
      }
  ...
  updateElementTypeCheckboxArrays(container, type) {
    try {
      const checkboxSelector = type === 'allowed' ? 
  ...
  resetFormFields(container) {
    try {
      ...
      const blacklistedValuesTextarea = container.querySelector('[name="filter.blacklistedValues"]');
      ...
      const whitelistMatchMode = container.querySelector('[name="filter.valueMatchMode.whitelist"]');
      if (whitelistMatchMode) {
        whitelistMatchMode.value = "exact";
        console.log("✅ Whitelist match mode reset to 'exact'");
      }

      const blacklistMatchMode = container.querySelector('[name="filter.valueMatchMode.blacklist"]');
      ...
      const mediaIndexBlacklisted = container.querySelector('[name="filter.mediaIndexes.blacklisted"]');
      ...
      const elementTypesAllowed = container.querySelector('[name="filter.elementTypes.allowedTypes"]');
      if (elementTypesAllowed) {
        elementTypesAllowed.value = "";
        console.log("✅ Element types allowed cleared");
      }

      const elementTypesBlacklisted = container.querySelector('[name="filter.elementTypes.blacklistedTypes"]');
      ...
      const elementLabelsAllowed = container.querySelector('[name="filter.elementLabels.allowedValues"]');
      if (elementLabelsAllowed) {
        elementLabelsAllowed.value = "";
        console.log("✅ Element labels allowed cleared");
      }

      const elementLabelsBlacklisted = container.querySelector('[name="filter.elementLabels.blacklistedValues"]');
      ...
      const tagFilteringAllowed = container.querySelector('[name="filter.tagFiltering.allowedTags"]');
      if (tagFilteringAllowed) {
        tagFilteringAllowed.value = "";
        console.log("✅ Tag filtering allowed cleared");
      }

      const tagFilteringBlacklisted = container.querySelector('[name="filter.tagFiltering.blacklistedTags"]');
  ...
  updateConfigFromForm(container) {
    try {
      ...
      if (tabContainer) {
        ...
        if (!this.core.config.filter?.elementTypes?.allowedTypes || 
        ...
        if (!this.core.config.filter?.elementTypes?.blacklistedTypes || 
      ...
      if (this.core.config.filter) {
        ...
        if (typeof this.core.config.filter.allowedValues === 'string') {
          this.core.config.filter.allowedValues = this.core.config.filter.allowedValues
            ...
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted allowedValues from string to array:", this.core.config.filter.allowedValues);
        }
        ...
        if (typeof this.core.config.filter.blacklistedValues === 'string') {
          this.core.config.filter.blacklistedValues = this.core.config.filter.blacklistedValues
            ...
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted blacklistedValues from string to array:", this.core.config.filter.blacklistedValues);
        }

        // Convert allowedTags from string to array if needed
        if (typeof this.core.config.filter.tagFiltering?.allowedTags === 'string') {
          this.core.config.filter.tagFiltering.allowedTags = this.core.config.filter.tagFiltering.allowedTags
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted allowedTags from string to array:", this.core.config.filter.tagFiltering.allowedTags);
        }
        ...
        if (typeof this.core.config.filter.tagFiltering?.blacklistedTags === 'string') {
          this.core.config.filter.tagFiltering.blacklistedTags = this.core.config.filter.tagFiltering.blacklistedTags
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted blacklistedTags from string to array:", this.core.config.filter.tagFiltering.blacklistedTags);
        }

        // Convert media index arrays from string to array if needed
        if (typeof this.core.config.filter.mediaIndexes?.allowed === 'string') {
          this.core.config.filter.mediaIndexes.allowed = this.core.config.filter.mediaIndexes.allowed
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted mediaIndexes.allowed from string to array:", this.core.config.filter.mediaIndexes.allowed);
        }

        if (typeof this.core.config.filter.mediaIndexes?.blacklisted === 'string') {
          this.core.config.filter.mediaIndexes.blacklisted = this.core.config.filter.mediaIndexes.blacklisted
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted mediaIndexes.blacklisted from string to array:", this.core.config.filter.mediaIndexes.blacklisted);
        }
        ...
        if (typeof this.core.config.filter.elementLabels?.allowedValues === 'string') {
          this.core.config.filter.elementLabels.allowedValues = this.core.config.filter.elementLabels.allowedValues
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted elementLabels.allowedValues from string to array:", this.core.config.filter.elementLabels.allowedValues);
        }

        if (typeof this.core.config.filter.elementLabels?.blacklistedValues === 'string') {
          this.core.config.filter.elementLabels.blacklistedValues = this.core.config.filter.elementLabels.blacklistedValues
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted elementLabels.blacklistedValues from string to array:", this.core.config.filter.elementLabels.blacklistedValues);
        }
        ...
        if (typeof this.core.config.filter.elementTypes?.allowedTypes === 'string') {
          this.core.config.filter.elementTypes.allowedTypes = this.core.config.filter.elementTypes.allowedTypes
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted elementTypes.allowedTypes from string to array:", this.core.config.filter.elementTypes.allowedTypes);
        }

        if (typeof this.core.config.filter.elementTypes?.blacklistedTypes === 'string') {
          this.core.config.filter.elementTypes.blacklistedTypes = this.core.config.filter.elementTypes.blacklistedTypes
            .split(',')
            .map(val => val.trim())
            .filter(val => val !== '');
          console.log("🔧 FILTER FIX: Converted elementTypes.blacklistedTypes from string to array:", this.core.config.filter.elementTypes.blacklistedTypes);
        }

        // Now clean up arrays if they exist (remove empty strings)
        if (Array.isArray(this.core.config.filter.blacklistedValues)) {
          this.core.config.filter.blacklistedValues = this.core.config.filter.blacklistedValues.filter(val => val && val.trim() !== '');
        }
        if (Array.isArray(this.core.config.filter.allowedValues)) {
          this.core.config.filter.allowedValues = this.core.config.filter.allowedValues.filter(val => val && val.trim() !== '');
        }
        ...
        if (this.core.config.filter.tagFiltering) {
          if (Array.isArray(this.core.config.filter.tagFiltering.blacklistedTags)) {
            this.core.config.filter.tagFiltering.blacklistedTags = this.core.config.filter.tagFiltering.blacklistedTags.filter(val => val && val.trim() !== '');
          }
          if (Array.isArray(this.core.config.filter.tagFiltering.allowedTags)) {
            this.core.config.filter.tagFiltering.allowedTags = this.core.config.filter.tagFiltering.allowedTags.filter(val => val && val.trim() !== '');
          }
        }
        ...
        if (this.core.config.filter.elementLabels) {
          if (Array.isArray(this.core.config.filter.elementLabels.blacklistedValues)) {
            this.core.config.filter.elementLabels.blacklistedValues = this.core.config.filter.elementLabels.blacklistedValues.filter(val => val && val.trim() !== '');
          }
          if (Array.isArray(this.core.config.filter.elementLabels.allowedValues)) {
            this.core.config.filter.elementLabels.allowedValues = this.core.config.filter.elementLabels.allowedValues.filter(val => val && val.trim() !== '');
          }
        }
        ...
        if (this.core.config.filter.mediaIndexes) {
          if (Array.isArray(this.core.config.filter.mediaIndexes.blacklisted)) {
            this.core.config.filter.mediaIndexes.blacklisted = this.core.config.filter.mediaIndexes.blacklisted.filter(val => val && val.trim() !== '');
          }
          if (Array.isArray(this.core.config.filter.mediaIndexes.allowed)) {
            this.core.config.filter.mediaIndexes.allowed = this.core.config.filter.mediaIndexes.allowed.filter(val => val && val.trim() !== '');
          }
        }
        ...
        if (this.core.config.filter.elementTypes) {
          if (Array.isArray(this.core.config.filter.elementTypes.blacklistedTypes)) {
            this.core.config.filter.elementTypes.blacklistedTypes = this.core.config.filter.elementTypes.blacklistedTypes.filter(val => val && val.trim() !== '');
          }
          if (Array.isArray(this.core.config.filter.elementTypes.allowedTypes)) {
            this.core.config.filter.elementTypes.allowedTypes = this.core.config.filter.elementTypes.allowedTypes.filter(val => val && val.trim() !== '');
          }
        }
        ...
        if (this.core.config.filter.elementTypes) {
          Object.keys(this.core.config.filter.elementTypes).forEach(key => {
            if (Array.isArray(this.core.config.filter.elementTypes[key])) {
              this.core.config.filter.elementTypes[key] = this.core.config.filter.elementTypes[key].filter(val => val && val.trim() !== '');
            }
          });
        }
      }
