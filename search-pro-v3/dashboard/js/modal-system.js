...
class SecureModalSystem {
  ...
  buildSafeHTML(htmlString) {
    ...
    if (!this.isContentSafe(htmlString)) {
      console.warn(
        "🚨 Security: Dangerous content detected in modal, rendering as plain text",
      );
    ...
    try {
      ...
      console.error(
        "🚨 Security: HTML parsing failed, falling back to text",
        error,
      );
  ...
  parseSafeModalHTML(htmlString, container) {
    ...
    const warningBoxRegex =
      /<div class="modal-warning-box">([\s\S]*?)<\/div>/gi;
    ...
    const cleanedContent = remainingContent
      .replace(/<\/?div[^>]*>/gi, "")
      .trim();
    if (cleanedContent) {
  ...
  createSafeList(listContent) {
    ...
    while ((liMatch = liRegex.exec(listContent)) !== null) {
      ...
      const remainingText = processedItemContent
        .replace(/<\/?[^>]+>/gi, "")
        .trim();
      if (remainingText) {
        const textNode = document.createTextNode(
          this.sanitizeInput(remainingText),
        );
  ...
  setupEventListeners() {
    ...
    if (!overlay || !confirmBtn || !cancelBtn) {
      console.warn(
        "⚠️ Modal elements not found - deferring event listener setup",
      );
      return;
    }
  ...
  showModal(options) {
    ...
    try {
      ...
      if (
        ...
        originalSubtitle !== sanitizedSubtitle
      ) {
      ...
      if (!overlay) {
        console.error(
          "❌ Modal overlay not found! Creating modal container...",
        );
        this.createModalContainer();
      ...
      if (
        ...
        console.log(
          `⏰ Setting up auto-close timer for ${autoCloseDelay}ms (type: ${type})`,
        );
        this.autoCloseTimer = setTimeout(() => {
          console.log(
            `🔄 Auto-closing ${type} modal after ${autoCloseDelay}ms`,
          );
          if (
            this.currentModal &&
            document.getElementById("modalOverlay").classList.contains("active")
...
<div class="modal-info-box">
  ...
  showLoadConfigModal(onConfirm, onCancel) {
    this.showModal({
      ...
      content:
        "Loading a configuration file will replace all current settings.",
      details: `
                <div class="modal-warning-box">
