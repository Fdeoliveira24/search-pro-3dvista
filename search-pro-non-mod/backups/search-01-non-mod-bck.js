...
window.tourSearchFunctions = (function () {
  ...
  class ConfigBuilder {
        ...
        searchField: {
          ...
          typography: {
            ...
            focus: {
              ...
              fontWeight: "500", // *** Font weight when focused
        ...
        setSearchFieldTypography(options) {
          ...
          this.config.appearance.searchField.typography = {
            ...
            focus: {
              ...
              fontWeight: options.focus?.fontWeight || "500",
    ...
    setIconSettings(options) {
      ...
      // Valid icon sizes mapping
      const iconSizeMap = {
        small: 16,
        medium: 20,
        large: 24,
        custom: options.iconSizePx || 20,
      };
      ...
      // Resolve icon size
      let sizePx = iconSizeMap[options.iconSize] || iconSizeMap.medium;
      if (options.iconSize === "custom" && options.iconSizePx) {
        sizePx = options.iconSizePx;
      }
  ...
  function _shouldIncludeElement(elementType, label, tags) {
    try {
      ...
      // [4.11.2.3] Apply element type filtering
      ...
      // [4.11.2.4] NEW - Apply unique item name filtering (exact match)
      if (label) {
      ...
      // [4.11.2.5] Apply label filtering
  ...
  function _applySearchStyling() {
    ...
    // [4.13.1.9.3.1] Always set the current size
    root.style.setProperty("--thumbnail-current-size", thumbSize + "px");

    // [4.13.1.9.3.2] Only update predefined sizes if using a standard size name AND custom pixel size
    if (_config.thumbnailSettings?.thumbnailSizePx) {
      ...
      // [4.13.1.9.3.3] Add data attribute to indicate custom sizing
    ...
    root.style.setProperty(
      ...
      searchTypography.fontSize || "16px",
    );
    root.style.setProperty(
      "--search-input-font-family",
      searchTypography.fontFamily || "inherit",
    );
    root.style.setProperty(
      "--search-input-font-weight",
      searchTypography.fontWeight || "400",
    );
    root.style.setProperty(
      "--search-input-font-style",
      searchTypography.fontStyle || "normal",
    );
    root.style.setProperty(
      "--search-input-line-height",
      searchTypography.lineHeight || "1.5",
    );
    root.style.setProperty(
      "--search-input-letter-spacing",
      searchTypography.letterSpacing || "0px",
    );
    root.style.setProperty(
      "--search-input-text-transform",
      searchTypography.textTransform || "none",
    );
    ...
    root.style.setProperty(
      ...
      focusTypography.fontWeight || searchTypography.fontWeight || "500",
    );
    root.style.setProperty(
...
.result-item .highlight {
  ...
  function processGoogleSheetsData(fuseData, config) {
    ...
    _googleSheetsData.forEach((sheetsEntry, sheetsIndex) => {
      try {
        ...
        // [5.2.3.7] Filter the entry based on inclusion rules
  ...
  function _initializeSearch(tour) {
          ...
          config.includeContent.containerSearch.containerNames.forEach(
            (containerName, containerIndex) => {
              try {
                ...
                // Filter container based on inclusion rules
                if (
                  !_shouldIncludeElement(elementType, displayLabel, elementTags)
                ) {
                  Logger.debug(
    ...
    function process3DModel(
      ...
      // [6.4.1] Filter 3D model based on configuration
      if (
      ...
      // [6.4.2] Add 3D model to search index
      fuseData.push({
        type: "3DModel",
        source: source,
        index,
      ...
      if (Array.isArray(objects)) {
        ...
        objects.forEach((obj, objIdx) => {
          ...
          // [6.4.3.4] Filter object based on configuration
    ...
    function processPanorama(
      ...
      // [6.5.1] Filter panorama based on configuration
      if (
        !_shouldIncludePanorama(
          label,
          subtitle,
      ...
      // [6.5.2] Match with business and sheets data
      const businessMatch = getBusinessMatch(label, media, tags, config);
      const sheetsMatch = getSheetsMatch(label, media, tags, config);

      // [6.5.3] Determine final display label
      ...
      // [6.5.5] Add panorama to search index
      fuseData.push({
        type: "Panorama",
        source: source,
        index,
      ...
      // [6.5.6] Process panorama overlays
    ...
    function _processOverlays(overlays, fuseData, parentIndex, parentLabel) {
      ...
      overlays.forEach((overlay, overlayIndex) => {
        try {
          ...
          // [6.17.8] Create a fallback label if needed
      ...
      window.searchFunctions.updateConfig({
        ...
        appearance: {
          searchField: {
            ...
            typography: {
              ...
              focus: {
                fontSize: "16px", // *** Font size when focused
                fontWeight: "900", // *** Font weight when focused
                letterSpacing: "0.25px", // *** Letter spacing when focused
              },
            },
          },
        ...
        thumbnailSettings: {
          ...
          defaultImages: {
            ...
            ProjectedImage:
              "./search-pro-non-mod/assets/projected-image-default.jpg", // *** Projected image default thumbnail path
            ...
            "3DModelObject":
              "./search-pro-non-mod/assets/3d-model-object-default.jpg", // *** 3D model object default thumbnail path
          ...
          iconSettings: {
            ...
            // BASIC ICON SETTINGS
            // ==========================================
            iconSize: "medium", // "small" (16px), "medium" (20px), "large" (24px), "custom"
            iconSizePx: 20, // *** Custom size in pixels (only used when iconSize: "custom")
        ...
        includeContent: {
          ...
          containerSearch: {
            ...
            containerNames: ["My_Container", "TwinsViewer-Container"], // Array of container names to include in search results i.e "My_Container","TwinsViewer-Container"
          },
        },
