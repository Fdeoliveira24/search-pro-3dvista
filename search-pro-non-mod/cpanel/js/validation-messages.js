...
class SecureValidationManager {
  ...
  sanitizeInput(input) {
    ...
    if (sanitized.length > this.maxMessageLength) {
      console.warn(
        "🚨 Security: Validation message truncated due to length limit",
      );
  ...
  showValidationMessage(field, type, message, duration = 3000) {
    try {
      ...
      if (this.activeMessages.size >= this.maxActiveMessages) {
        console.warn(
          "🚨 Security: Maximum validation messages reached, preventing spam",
        );
        return;
      }
      ...
      const safeDuration = Math.max(
        1000,
        Math.min(30000, Number(duration) || 3000),
      );
      ...
      if (!formGroup) {
        console.warn(
          "🚨 Security: Form group not found for validation message",
        );
        return;
      }
      ...
      const messageElement = this.createValidationElement(
        validatedType,
        sanitizedMessage,
        fieldId,
      );
  ...
  clearAllValidationMessages() {
    try {
      ...
      console.error(
        "🚨 Security: Error clearing all validation messages:",
        error,
      );
    }
  }
  ...
  getMessagesForField(fieldId) {
    const safeFieldId = this.sanitizeInput(fieldId);
    return this.activeMessages.has(safeFieldId)
      ? this.activeMessages.get(safeFieldId)
      : null;
  }
    ...
    this.activeMessages.forEach((messageElement, fieldId) => {
      try {
        ...
        if (textElement && !this.isContentSafe(textElement.textContent)) {
          console.warn(
            "🚨 Security: Unsafe content detected in active message",
          );
          this.clearValidationMessage(fieldId);
          issuesFound++;
        }
  ...
  emergencyCleanup() {
    try {
      console.warn(
        "🚨 Security: Performing emergency cleanup of validation system",
      );
...
window.showValidationMessage = function (field, type, message, duration) {
  try {
    window.validationManager.showValidationMessage(
      field,
      type,
      message,
      duration,
...
window.ValidationSecurity = {
  ...
  validateParams(field, type, message) {
    ...
    if (!isValid) {
      console.warn(
        "🚨 Security: Potentially unsafe validation parameters blocked",
      );
      return false;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  console.log(
    "🛡️ Secure Validation System loaded with the following protections:",
  );
