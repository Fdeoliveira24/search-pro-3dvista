...
class SecureLogoManager {
  ...
  createFileUploadSection() {
    ...
    helpText.textContent =
      "JPG, PNG, SVG, WebP • Max 2MB • Square format recommended";
  ...
  createUrlInputSection(currentLogoSrc) {
    ...
    // Safe URL handling
    const safeCurrentUrl = this.isDataUrl(currentLogoSrc)
      ? ""
      : currentLogoSrc || "";
  ...
  showLogoModal() {
    if (
      ...
      !window.searchProControlPanel.modalSystem
    ) {
    ...
    try {
      ...
      const safeOriginalBrandText = this.sanitizeInput(
        originalBrandText,
        this.maxBrandTextLength,
      );
        ...
        onConfirm: () => {
          try {
            ...
            if (!this.isContentSafe(newBrandText)) {
              console.warn(
                "🚨 Security: Brand text contains unsafe content, using fallback",
              );
            ...
            if (tempLogoSrc !== safeOriginalLogoSrc) {
              ...
              if (
                this.isValidUrl(safeTempLogoSrc) ||
                this.isDataUrl(safeTempLogoSrc)
              ) {
            ...
            console.error(
              "🚨 Security: Error saving branding settings:",
              error,
            );
            if (window.toastManager) {
              window.toastManager.showToast(
      ...
      if (window.toastManager) {
        window.toastManager.showToast(
          "error",
          "Modal Error",
          "Failed to open branding settings",
        );
      }
    }
  }
    ...
    try {
      ...
      if (brandTextInput) {
        brandTextInput.addEventListener("input", (e) => {
          try {
            const sanitizedValue = this.sanitizeInput(
              e.target.value,
              this.maxBrandTextLength,
            );
            ...
            console.error(
              "🚨 Security: Error handling brand text input:",
              error,
            );
          }
        });
      ...
      console.error(
        "🚨 Security: Error setting up modal event handlers:",
        error,
      );
    }
  }
  ...
  loadBrandingPreferences() {
    try {
      ...
      if (
        savedLogo &&
        (this.isValidUrl(savedLogo) || this.isDataUrl(savedLogo))
      ) {
...
class LogoManager extends SecureLogoManager {
  constructor() {
    super();
    console.log(
      "🛡️ Secure Logo Manager initialized with XSS protection and DOM integration fix",
    );
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  console.log(
    "🛡️ Fixed Secure Logo Manager loaded with the following protections:",
  );
