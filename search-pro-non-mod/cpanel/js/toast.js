  ...
  showToast(type, title, message, duration = 5000) {
    try {
      ...
      const safeDuration = Math.max(
        1000,
        Math.min(30000, Number(duration) || 5000),
      );
      ...
      const contentElement = this.createToastContent(
        sanitizedTitle,
        sanitizedMessage,
      );
...
window.ToastSecurity = {
  ...
  validateToastParams(type, title, message) {
    const isValid =
      window.toastManager.isContentSafe(title) &&
      window.toastManager.isContentSafe(message);
