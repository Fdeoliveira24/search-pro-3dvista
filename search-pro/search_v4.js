...
window.tourSearchFunctions = (function() {
    ...
    class ConfigBuilder {
        ...
        setThemeCustomization(options) {
            ...
            this.config.theme = {
                useDarkMode: options?.useDarkMode !== undefined ? options.useDarkMode : null,
                useSystemPreference: options?.useSystemPreference !== undefined ? options.useSystemPreference : true,
        ...
        validate() {
            ...
            if (typeof this.config.minSearchChars !== 'number' || this.config.minSearchChars < 1) {
            ...
            if (this.config.searchBar && this.config.searchBar.position) {
                ...
                if (position.top !== null && (typeof position.top !== 'number' || position.top < 0)) {
                    errors.push('position.top must be a non-negative number');
                }
                if (position.right !== null && (typeof position.right !== 'number' || position.right < 0)) {
                    errors.push('position.right must be a non-negative number');
                }
                if (position.left !== null && position.left !== '50%' && (typeof position.left !== 'number' || position.left < 0)) {
            ...
            if (this.config.theme && this.config.theme.typography) {
                ...
                if (typography.fontSize !== undefined && (typeof typography.fontSize !== 'number' || typography.fontSize < 8 || typography.fontSize > 30)) {
                    errors.push('typography.fontSize must be a number between 8 and 30');
                }
                
                if (typography.letterSpacing !== undefined && (typeof typography.letterSpacing !== 'number' || typography.letterSpacing < -5 || typography.letterSpacing > 10)) {
                    errors.push('typography.letterSpacing must be a number between -5 and 10');
                }
            }
            
            // NEW: Validate searchWidth
            if (this.config.appearance && this.config.appearance.searchWidth !== undefined) {
                const searchWidth = this.config.appearance.searchWidth;
                if (typeof searchWidth !== 'number' || searchWidth < 200 || searchWidth > 1000) {
                    errors.push('appearance.searchWidth must be a number between 200 and 1000');
                }
            }
            
            // NEW: Validate maxResultsHeight
            if (this.config.appearance && this.config.appearance.searchResults && 
                this.config.appearance.searchResults.maxHeight !== undefined) {
                const maxHeight = this.config.appearance.searchResults.maxHeight;
                if (typeof maxHeight !== 'number' || maxHeight < 100 || maxHeight > 2000) {
                    errors.push('appearance.searchResults.maxHeight must be a number between 100 and 2000');
                }
            }
            ...
            if (this.config.appearance && this.config.appearance.colors) {
                ...
                for (const prop of colorProps) {
                    ...
                    if (typeof color === 'string') {
                        ...
                        const validHex = /^#([A-Fa-f0-9]{3,4}|[A-Fa-f0-9]{6}|[A-Fa-f0-9]{8})$/.test(color);
                        
                        // Check for valid rgba format
                        const validRgba = /^rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*(?:,\s*(?:0?\.\d+|[01])\s*)?\)$/.test(color);
                        
                        if (!validHex && !validRgba) {
                            errors.push(`${prop} color (${color}) is not a valid hex or rgba format`);
                        }
            ...
            if (this.config.theme) {
                ...
                if (this.config.theme.contrastMode && !validContrastModes.includes(this.config.theme.contrastMode)) {
                    errors.push(`contrastMode must be one of: ${validContrastModes.join(', ')}`);
                }
                
                const validAnimationTypes = ['fade', 'slide', 'scale', 'none'];
                if (this.config.theme.animation && this.config.theme.animation.type && 
                    !validAnimationTypes.includes(this.config.theme.animation.type)) {
                    errors.push(`animation.type must be one of: ${validAnimationTypes.join(', ')}`);
                }
            }
    ...
    function _applySearchStyling() {
        ...
        if (_config.theme) {
            ...
            searchContainer.classList.remove('light-theme', 'dark-theme', 'high-contrast', 'low-contrast');
            ...
            if (_config.theme.animation && _config.theme.animation.speed) {
                searchContainer.setAttribute('data-animation-speed', _config.theme.animation.speed);
            }
            
            // Apply animation type if specified
            if (_config.theme.animation && _config.theme.animation.type) {
                searchContainer.setAttribute('data-animation-type', _config.theme.animation.type);
            }
        ...
        const typography = _config.theme && _config.theme.typography ? _config.theme.typography : {};
    ...
    function _initializeSearch(tour) {
        ...
        try {
            ...
            Logger.warn('[SearchPro] Failed to load saved configuration:', e);
        }
    ...
    minMatchCharLength: 2
        ...
        const performSearch = () => {
            ...
            try {
                ...
                resultsContainer.setAttribute('aria-label', `Found ${matches.length} search results for "${searchTerm}"`);
                ...
                `;
                // Set accessibility label for error state
